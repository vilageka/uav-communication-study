...
/*
 * UAV urban LTE infrastructure AoI experiment
 * -------------------------------------------
 *
 * Diese Datei uebertraegt das urbane Gebaeudeszenario auf die LTE-basierte
 * Infrastrukturarchitektur der Bachelorarbeit.
 *
 ...
 * - UAVs und eNodeB stehen in einer parametrisierbaren Block-Stadt.
 * - Der LTE-Funkkanal nutzt HybridBuildingsPropagationLossModel.
 *
 ...
 */
NS_LOG_COMPONENT_DEFINE("UavUrbanLteInfrastructureAoi");
...
struct UrbanScenarioConfig
{
    ...
    std::string buildingMetricsFile{"uav-urban-lte-buildings.csv"};
};
...
Vector
CreateCentralStreetPosition(double altitudeMeters, const UrbanScenarioConfig& urban)
{
    const uint32_t centralStreetX = std::max<uint32_t>(1, urban.blocksX / 2);
    const uint32_t centralStreetY = std::max<uint32_t>(1, urban.blocksY / 2);
    const double x =
        centralStreetX * urban.buildingLengthX + (centralStreetX + 0.5) * urban.streetWidth;
    const double y =
        centralStreetY * urban.buildingLengthY + (centralStreetY + 0.5) * urban.streetWidth;

    return Vector(x, y, altitudeMeters);
}
...
void
MakeBuildingInfoConsistent(const NodeContainer& nodes)
{
    for (uint32_t i = 0; i < nodes.GetN(); ++i)
    {
        ...
        NS_ABORT_MSG_UNLESS(buildingInfo, "Missing MobilityBuildingInfo for node " << i);
...
main(int argc, char* argv[])
{
    ...
    MakeBuildingInfoConsistent(uavs);
    ...
    enbPosition->Add(CreateCentralStreetPosition(enbHeightMeters, urban));
    MobilityHelper enbMobility;
    enbMobility.SetPositionAllocator(enbPosition);
    enbMobility.SetMobilityModel("ns3::ConstantPositionMobilityModel");
    enbMobility.Install(enbNodes);
    BuildingsHelper::Install(enbNodes);
    MakeBuildingInfoConsistent(enbNodes);
    ...
    lteHelper->SetPathlossModelAttribute("RooftopLevel", DoubleValue(urban.buildingHeight));
    ...
    std::cout << "Architecture: Urban LTE infrastructure with AoI sampling" << std::endl;
